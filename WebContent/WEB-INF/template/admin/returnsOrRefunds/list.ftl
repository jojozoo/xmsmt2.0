...
<body>
	...
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; 退款（货）中订单 <span>(${message("admin.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="returnOrRefundsList.jhtml" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as order]
				<tr>
					...
					[/@shiro.hasPermission]
					<td>
						<a href="viewReturnsOrRefunds.jhtml?id=${order.id}">[${message("admin.common.view")}]</a>
						[#if !order.expired && order.orderStatus == "unconfirmed"]
							<a href="edit.jhtml?id=${order.id}">[${message("admin.common.edit")}]</a>
						[/#if]
						[#list order.returns as returns]
							[#if !order.expired && order.shippingStatus == "apply"]
								<a href="../order/viewReturns.jhtml?id=${returns.id}">[退货确认]</a>
							[/#if]
						[/#list]
						[#list order.refunds as refund]
							[#if refund.status == "apply"]
								<a href="../refunds/view.jhtml?id=${refund.id}">[退款确认]</a>
							[/#if]
						[/#list]
					</td>
				</tr>
