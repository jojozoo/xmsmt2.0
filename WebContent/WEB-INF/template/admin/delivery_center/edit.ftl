		...
		if ($areaId.val()>0) {
			$.ajax({
				...
				success: function(map) {
					...
					for (var key in map) {
						[#if deliveryCenter.community??]
						if(key == ${deliveryCenter.community.id}) {
							opt = opt +"<option value="+key+" selected=selected>"+map[key]+"</option>";
						} else {
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
			<tr>
				...
				<td>
					<input type="text" name="sn" class="text" value="${deliveryCenter.sn}" maxlength="200" />
				</td>
			</tr>
			<tr>
				<th>
			...
			<!--<tr>
				...
				<td >
					<input type="hidden" name="locationX" id="locationX" class="text" value="[#if deliveryCenter.location??]${deliveryCenter.location.x}[/#if]"/>
					<input type="hidden" name="locationY" id="locationY" class="text" value="[#if deliveryCenter.location??]${deliveryCenter.location.y}[/#if]"/>
					<span id="location">[#if deliveryCenter.location??](x:${deliveryCenter.location.x},y:${deliveryCenter.location.y})[/#if]</span><input type="button" class="button" onclick="openMap();" value="获取" />
				</td>
			</tr>-->
			<tr>
				<th>
...
<script type="text/javascript">
  ...
  [#if deliveryCenter.locationX == ""] 
     map.centerAndZoom("北京市", 18);  
  [#else]
     map.centerAndZoom(new BMap.Point(${deliveryCenter.locationX},${deliveryCenter.locationY}), 18);  
  [/#if]
