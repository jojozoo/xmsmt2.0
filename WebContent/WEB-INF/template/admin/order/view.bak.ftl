...
$().ready(function() {
	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.paymentStatus == "paid" || order.paymentStatus == "partialPayment" || order.paymentStatus == "partialRefunds" || order.paymentStatus == "refundapply")]
		// 退款
...
<body>
	...
	<ul id="tab" class="tab">
		...
		<li>
			<input type="button" value="${message("admin.order.tradeInfo")}" />
		</li>
		<li>
	...
	<table class="input tabContent">
		<tr>
			...
			<td>
				...
				<input type="button" id="shippingButton" class="button" value="${message("admin.order.shipping")}"[#if order.expired || order.orderStatus != "confirmed" || (order.shippingStatus != "unshipped" && order.shippingStatus != "partialShipment") || order.paymentStatus == "refundapply" || order.paymentStatus == "refunded"] disabled="disabled"[/#if] />
			...
			<td>
				<input type="button" id="refundsButton" class="button" paymentStatus="${order.paymentStatus}" value="${message("admin.order.refunds")}"[#if order.expired || order.orderStatus != "confirmed" || (order.paymentStatus != "paid" && order.paymentStatus != "partialPayment" && order.paymentStatus != "partialRefunds" && order.paymentStatus != "refundapply")] disabled="disabled"[/#if] />
		...
		[#if order.invoiceManagement!=null]
			<tr>
				<th>
					${message("Order.invoiceTitle")}:
				</th>
				<td>
					${order.invoiceManagement.invoiceTitle}
				</td>
			</tr>
		[/#if]
		<tr>
