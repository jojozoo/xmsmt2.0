	...
	[#if !order.expired && order.orderStatus == "confirmed" && (order.shippingStatus == "unshipped" || order.shippingStatus == "partialShipment")]
		...
		$shippingButton.click(function() {
			$.dialog({
						...
						<div style="height: 380px; overflow-x: hidden; overflow-y: auto;">
							<table class="input" style="margin-bottom: 30px;">
								...
								<tr>
									...
									<\/th>
									<td>
										<select id="deliveryCorpId" name="deliveryCorpId">
										    <option value="">${message("admin.common.choose")}<\/option>
				...
				onOk: function() {
					 var deliveryCorp=$("#deliveryCorpId").val();
				     if(deliveryCorp==""){
				        alert("请选择物流公司！");
				        $(this).dialog("open");
				     }else{
...
<body>
	...
	<table class="input tabContent">
		...
		<tr>
			...
			<td>
			<form id="listForm" action="updat.jhtml" method="POST">
			    <input type="hidden" name="orderId" value="${order.id}" /> 
				<input type="text" name="memo" class="text" value="${order.memo}"  /><input type="submit" class="button" value="提交" />
			</form>
			</td>
		</tr>
	</table>
	...
	[#else]
		...
		<tr>
	     ...
	     <tr>
			...
			<td>
				[#if order.invoiceManagement.invoiceStat=="cancel"]
				  取消发票
				[/#if]
				[#if order.invoiceManagement.invoiceStat=="yes"]
				  已开票
				[/#if]
			</td>
	     </tr>
	     <tr>
			...
			<td>
				${order.invoiceManagement.invoiceValue}
			</td>
		</tr>
	     <tr>
			<th>
			...
			<td>
			   [#if order.invoiceManagement.invoiceContent!=null]
				 ${order.invoiceManagement.invoiceContent.content}
			   [/#if]
			</td>
		</tr>
	     <tr>
	     ...
	     <tr>
			...
			<td>
				...
				<form id="listForm" action="updat.jhtml" method="POST">
			    <input type="hidden" name="orderId" value="${order.id}" /> 
				<input type="text" name="remark" class="text" value="${order.invoiceManagement.remark}"  /><input type="submit" class="button" value="提交" />
			</form>
			</td>
		</tr>
	[/#if]
	...
	<table class="input">
		<tr>
			...
			<td>
				<input type="button" class="button" value="${message("admin.common.back")}" onclick="location.href='list.jhtml?flag=${flag}'" />
			</td>
		</tr>
	</table>
</body>
