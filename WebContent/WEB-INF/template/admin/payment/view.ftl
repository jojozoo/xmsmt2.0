...
<title>${message("admin.payment.view")} - Powered By rsico</title>
...
<body>
	...
	<table class="input">
		...
		<tr>
			...
			<td>
				${(payment.member.nickName)!"-"}
			</td>
			<th>
				${message("Payment.order")}:
			</th>
			<td>
			   <a href="orderView.jhtml?id=${payment.order.id}" style="color:red;">${(payment.order.sn)!"-"}</a>
			</td>
		</tr>
		<tr>
			<th>
