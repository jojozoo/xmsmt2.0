...
<title>${message("admin.shippingMethod.edit")} - Powered By rsico</title>
...
<body>
	...
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
			...
			<tr>
				<th>
					<span class="requiredField">*</span>配送类型:
				</th>
				<td>
					<select name="method">
						<option value="TPL" [#if ("TPL" == (shippingMethod.method)!)] selected[/#if]>三方物流</option>
						<option value="F2F" [#if ("F2F" == (shippingMethod.method)!)] selected[/#if]>验证发货</option>
						<option value="PRIVY" [#if ("PRIVY" == (shippingMethod.method)!)] selected[/#if]>专属配送</option>
					</select>
				</td>
			</tr>
			<tr>
