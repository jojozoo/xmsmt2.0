...
public class ArticleServiceImpl extends BaseServiceImpl<Article, Long> implements ArticleService, DisposableBean {
	...
	@Transactional(readOnly = true)
	public List<Article> findList(ArticleCategory articleCategory, List<Tag> tags, Tenant tenant, Area area, Integer count, List<Filter> filters, List<Order> orders) {
		return articleDao.findList(articleCategory, tags, tenant, area, count, filters, orders);
	}

	@Transactional(readOnly = true)
	@Cacheable("article")
	public List<Article> findList(ArticleCategory articleCategory, List<Tag> tags, Tenant tenant, Area area, Integer count, List<Filter> filters, List<Order> orders, String cacheRegion) {
		return articleDao.findList(articleCategory, tags, tenant, area, count, filters, orders);
	}
	...
	public Page<Article> findPage(Set<ArticleCategory> articleCategorys, List<Tag> tags, Area area, Pageable pageable) {
		return articleDao.findPage(articleCategorys, tags, area, pageable);
	}

	@Transactional(readOnly = true)
	public Page<Article> findMyPage(Tenant tenant, Pageable pageable) {
		return articleDao.findMyPage(tenant, pageable);
	}
