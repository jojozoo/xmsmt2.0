...
import net.wit.util.Configuration;
import net.wit.util.Digest;
...
public class ZGTService {
	 ...
	 * 短信验证码确认接口请求地址
	 */
	public static String getConfirmSmsURL() {
			return Configuration.getInstance().getValue("confirmSmsURL");
	}
	...
	public static String getQueryCardBinURL() {
			return Configuration.getInstance().getValue("queryCardBinURL");
	}	

	/**
	 * 对账文件下载请求地址
	 */
	public static String getClearDataURL() {
			return Configuration.getInstance().getValue("clearDataURL");
	}	
	...
	public static Map<String, String> parseHttpResponseBody(int statusCode, String responseBody,
		...
		System.out.println("dataFromYeepay=" + dataFromYeepay);
	...
	public static Map<String, String> updateAccount(Map<String, String> params) {
        ...
        String bindmobile          	= formatString(params.get("bindmobile"));          
	...
	public static Map<String, String> paymentRequest(Map<String, String> reqParams) {
		...
		System.out.println("get请求串：" + requestURL + "?customernumber=" + customernumber + "&data=" + data);
	...
	public static Map<String, String> sendSms(String orderrequestid) {
		...
		System.out.println("##### sendSms() #####");
	 	...
	 	String keyForHmac 				= getKeyValue();
	 	String keyForAES 				= getKeyValue().substring(0, 16);
		String sendSmsURL				= getSendSmsURL();

		StringBuffer buffer			= new StringBuffer();
		buffer.append(customernumber);
		buffer.append(orderrequestid);
		...
		System.out.println("dataMap=" + dataMap);
		System.out.println("sendSmsURL=" + sendSmsURL);
	...
	/**
	 * confirmSms() : 短信验证码确认接口
	 */

	public static Map<String, String> confirmSms(String orderrequestid, String smscode) {

		System.out.println("##### confirmSms() #####");

	 	String customernumber 			= getCustomernumber();
	 	String keyForHmac 				= getKeyValue();
	 	String keyForAES 				= getKeyValue().substring(0, 16);
		String confirmSmsURL			= getConfirmSmsURL();
		...
		System.out.println("dataMap=" + dataMap);
		System.out.println("confirmSmsURL=" + confirmSmsURL);
		System.out.println("customernumber=" + customernumber);
		System.out.println("data=" + data);

		HttpClient httpClient			= new HttpClient();
		PostMethod postMethod			= new PostMethod(confirmSmsURL);
		NameValuePair[] datas			= {new NameValuePair("customernumber", customernumber),
									       new NameValuePair("data", data)};
		...
		try {
			...
			String[] hmacEncryptionOrder = {"customernumber", "orderrequestid", "code", "amount", "bindid", "bankcode"};
...
//	/**
//	 * queryCardBin() : 卡Bin查询接口
//	 */
//
//	public static Map<String, String> queryCardBin(String bankaccountnumber) {
//
...
//		StringBuffer buffer			= new StringBuffer();
//		buffer.append(customernumber);
//		buffer.append(bankaccountnumber);
//		String hmac					= Digest.hmacSign(buffer.toString(), keyForHmac);
//
//	 	Map<String, String> dataMap = new HashMap<String, String>();
//	 	dataMap.put("customernumber", customernumber);
...
//
//		System.out.println("dataMap : " + dataMap);
//		System.out.println("queryCardBinURL : " + queryCardBinURL);
//
//		System.out.println("customernumber : " + customernumber);
//		System.out.println("data : " + data);
//		
//		HttpClient httpClient		= new HttpClient();
//		PostMethod postMethod		= new PostMethod(queryCardBinURL);
//		NameValuePair[] datas		= {new NameValuePair("customernumber", customernumber),
//									   new NameValuePair("data", data)};
//		
//		Map<String, String> result	= new HashMap<String, String>();
...
//			String responseBody	= new String(responseByte, "UTF-8");
// 
//			String[] hmacEncryptionOrder = {"customernumber", "bankaccountnumber", "code", "bankcode", 
//											"bankname", "cardname", "cardtype"};
//			result	= parseHttpResponseBody(statusCode, responseBody, hmacEncryptionOrder);
//			
//		} catch(Exception e) {
//			customError	= "Caught an Exception - " + e.toString();
...
//	} 
	...
	/**
	 * getPathOfClearData() : 下载对账文件
	 */

	public static Map<String, String> getPathOfClearData(String checkDate, String orderType, String fileType, String sysPath) {

		System.out.println("##### getPathOfClearData() #####");

	 	String operator_no	 		= getCustomernumber();
	 	String key		 			= getKeyValue();
		String clearDataURL		    = getClearDataURL();

		StringBuffer buffer			= new StringBuffer();
		buffer.append(operator_no);
		buffer.append(checkDate);
		buffer.append(orderType);
		String hmac					= Digest.hmacSign(buffer.toString(), key);

	 	Map<String, String> certify_token_map = new HashMap<String, String>();
	 	certify_token_map.put("operator_no", operator_no);
	 	certify_token_map.put("checkDate", checkDate);
	 	certify_token_map.put("orderType", orderType);
	 	certify_token_map.put("fileType", fileType);
	 	certify_token_map.put("hmac", hmac);
	 	
	 	String certify_token_JsonString		= JSON.toJSONString(certify_token_map);
	 	
		String certify_token		= AESUtil.encrypt(certify_token_JsonString, key);
		String authorize_no			= getCustomernumber();

		System.out.println("certify_token_map : " + certify_token_map);
		System.out.println("clearDataURL : " + clearDataURL);
		System.out.println("certify_token : " + certify_token);
		System.out.println("authorize_no : " + authorize_no);
		
		HttpClient httpClient		= new HttpClient();
		PostMethod postMethod		= new PostMethod(clearDataURL);
		NameValuePair[] datas		= {new NameValuePair("certify_token", certify_token),
									   new NameValuePair("authorize_no", authorize_no)};
		
		Map<String, String> result	= new HashMap<String, String>();
		String customError			= ""; // 自定义变量，非接口返回
		String filePath				= ""; // 自定义变量，非接口返回
		...
		try {
			...
			if(statusCode != 200) {
				customError = "Post request failed. http code = " + statusCode;
				result.put("customError", customError);
				return result;
			}
			...
			String fileName					= "clearData" + "_" + time + "." + fileType;
			String absolutePath				= sysPath + File.separator + fileName;
			File file						= new File(absolutePath);
			file.getParentFile().mkdirs();
			file.createNewFile();

			FileOutputStream clearDataFile 	= new FileOutputStream(absolutePath);
