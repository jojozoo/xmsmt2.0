 ...
 * @version 3.0
 */
@Controller("adminUnionpayMobileController")
@RequestMapping("/admin/payment_plugin/unionpay/mobile")
public class UnionpayMobileController extends BaseController {

	@Resource(name = "unionpayMobilePlugin")
	private UnionpayMobilePlugin unionpayMobilePlugin;
	...
	public @ResponseBody
	Message install() {
		if (!unionpayMobilePlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(unionpayMobilePlugin.getId());
	...
	Message uninstall() {
		if (unionpayMobilePlugin.getIsInstalled()) {
			PluginConfig pluginConfig = unionpayMobilePlugin.getPluginConfig();
