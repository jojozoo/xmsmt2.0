package net.wit.plugin.alipayMobile;
...
public class AlipayMobilePlugin extends PaymentPlugin {
	...
	public Map<String, Object> getParameterMap(String sn, String description, HttpServletRequest request, String root) {
		...
		parameterMap.put("return_url", getNotifyUrl(sn, NotifyMethod.sync, root));
		parameterMap.put("notify_url", getNotifyUrl(sn, NotifyMethod.async, root));
		...
		parameterMap.put("extra_common_param", "zjng");
	...
	public boolean verifyNotify(String sn, NotifyMethod notifyMethod, HttpServletRequest request) {
		...
		if (generateSign(request.getParameterMap()).equals(request.getParameter("sign")) && pluginConfig.getAttribute("partner").equals(request.getParameter("seller_id")) && sn.equals(request.getParameter("out_trade_no"))
				&& ("TRADE_SUCCESS".equals(request.getParameter("trade_status")) || "TRADE_FINISHED".equals(request.getParameter("trade_status"))) && payment.getAmount().compareTo(new BigDecimal(request.getParameter("total_fee"))) == 0) {
			Map<String, Object> parameterMap = new HashMap<String, Object>();
