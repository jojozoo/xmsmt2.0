 ...
 * @version 3.0
 */
@Controller("adminChinapayController")
@RequestMapping("/admin/payment_plugin/chinapay")
public class ChinapayController extends BaseController {

	@Resource(name = "chinapayPlugin")
	private ChinapayPlugin chinapayPlugin;
	...
	public @ResponseBody
	Message install() {
		if (!chinapayPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(chinapayPlugin.getId());
	...
	Message uninstall() {
		if (chinapayPlugin.getIsInstalled()) {
			PluginConfig pluginConfig = chinapayPlugin.getPluginConfig();
	...
	public String setting(ModelMap model) {
		...
		return "/net/wit/plugin/unionpay/setting";
	}
