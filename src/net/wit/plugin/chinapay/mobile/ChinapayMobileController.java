 ...
 * @version 3.0
 */
@Controller("adminChinapayMobileController")
@RequestMapping("/admin/payment_plugin/chinapay/mobile")
public class ChinapayMobileController extends BaseController {

	@Resource(name = "chinapayMobilePlugin")
	private ChinapayMobilePlugin chinapayMobilePlugin;
	...
	public @ResponseBody
	Message install() {
		if (!chinapayMobilePlugin.getIsInstalled()) {
			PluginConfig pluginConfig = new PluginConfig();
			pluginConfig.setPluginId(chinapayMobilePlugin.getId());
	...
	Message uninstall() {
		if (chinapayMobilePlugin.getIsInstalled()) {
			PluginConfig pluginConfig = chinapayMobilePlugin.getPluginConfig();
	...
	public String setting(ModelMap model) {
		...
		return "/net/wit/plugin/unionpay/mobile/setting";
	}
